...
def main():
    ...
    input_shape = (opt.channels, img_resize_height, img_resize_width)
    ...
    def sample_images(batches_done):
        ...
        recon_A = make_grid(recon_A, nrow=5, normalize=True)
        # Arange images along y-axis
        image_grid = torch.cat((real_A, fake_B, recon_A, real_A), 1)
        save_image(image_grid, "images/%s/%s.png" % (opt.dataset_name, batches_done), normalize=False)

    def sample_images2(batches_done):
        """Saves a generated sample from the test set"""
        imgs = next(iter(val_dataloader))
        G_AB.eval()
        G_BA.eval()
